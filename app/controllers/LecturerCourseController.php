...
class LecturerCourseController extends BaseController {
    ...
    public function dashboard() {
        ...
        // Log for debugging
        error_log("Loading course dashboard for course_id: $courseId, lecturer_id: $lecturerId");
        ...
        if (!$course) {
            error_log("Course not found: course_id=$courseId, lecturer_id=$lecturerId");
            $this->setFlashMessage('Course not found or access denied', 'error');
            $this->redirect('/lecturer/dashboard');
            return;
        }
        
        error_log("Course loaded: " . $course['course_code'] . " (ID: " . $course['id'] . ")");
        ...
        $this->render('lecturer/course-dashboard', [
            ...
            'stats' => $stats,
            'courseId' => $courseId,
            'requestTime' => time(), // Add timestamp to force unique page
            'additionalCss' => '<link rel="stylesheet" href="' . asset('css/lecturer-dashboard.css') . '?v=' . time() . '">'
        ]);
    }
    ...
    public function store() {
        ...
        if (!$lecturer) {
            ...
            if ($this->isAjax()) {
                $this->jsonResponse(['success' => false, 'message' => 'Invalid lecturer profile. Please logout and login again.']);
            } else {
                $this->setFlashMessage('Invalid lecturer profile. Please logout and login again.', 'error');
                $this->redirect('/lecturer/dashboard');
            }
            return;
        }
        ...
        try {
            ...
            if ($this->isAjax()) {
                $this->jsonResponse([
                    'success' => false, 
                    'message' => APP_ENV === 'development' ? 'Failed to add course: ' . $e->getMessage() : 'Failed to add course'
                ]);
            } else {
    ...
    public function delete() {
        ...
        $lecturerId = $_SESSION['profile_id'] ?? null;
        $courseId = $_POST['course_id'] ?? $_GET['id'] ?? null;
        ...
        if (!$course) {
            if ($this->isAjax()) {
                $this->jsonResponse(['success' => false, 'message' => 'Course not found or access denied']);
            } else {
                $this->setFlashMessage('Course not found or access denied', 'error');
                $this->redirect('/lecturer/dashboard');
            }
        ...
        // Check if there are any attendance sessions for this course
        $sessionCount = $this->db->queryOne("
            SELECT COUNT(*) as count FROM attendance_sessions WHERE course_id = ?
        ", [$courseId])['count'];
        
        // If there are sessions, archive instead of delete
        if ($sessionCount > 0) {
            try {
                $this->db->execute("
                    UPDATE courses 
                ...
                if ($this->isAjax()) {
                    $this->jsonResponse([
                        'success' => true,
                        'message' => 'Course has been archived (it has attendance records)'
                    ]);
                } else {
                    $this->setFlashMessage('Course "' . $course['course_code'] . '" has been archived (it has attendance records)', 'success');
                    $this->redirect('/lecturer/dashboard');
                }
                
            } catch (Exception $e) {
                error_log('Archive course error: ' . $e->getMessage());
                
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'message' => 'Failed to archive course']);
                } else {
                    $this->setFlashMessage('Failed to archive course', 'error');
                    $this->redirect('/lecturer/dashboard');
                }
            }
        } else {
            // No sessions, safe to permanently delete
            try {
                $this->db->execute("
                    DELETE FROM courses WHERE id = ? AND lecturer_id = ?
                ", [$courseId, $lecturerId]);
                ...
                if ($this->isAjax()) {
                    ...
                    $this->setFlashMessage('Course "' . $course['course_code'] . '" has been deleted successfully', 'success');
                    $this->redirect('/lecturer/dashboard');
                }
                ...
                if ($this->isAjax()) {
                    $this->jsonResponse(['success' => false, 'message' => 'Failed to delete course']);
                } else {
    ...
    public function archive() {
        ...
        try {
            ...
            if ($this->isAjax()) {
                ...
                $this->setFlashMessage('Course "' . $course['course_code'] . '" has been archived successfully', 'success');
                $this->redirect('/lecturer/dashboard');
            }
    ...
    public function unarchive() {
        ...
        try {
            ...
            if ($this->isAjax()) {
                ...
                $this->setFlashMessage('Course "' . $course['course_code'] . '" has been restored successfully', 'success');
                $this->redirect('/lecturer/dashboard');
            }
            
        } catch (Exception $e) {
            error_log('Unarchive course error: ' . $e->getMessage());
            
            if ($this->isAjax()) {
                $this->jsonResponse(['success' => false, 'message' => 'Failed to restore course']);
            } else {
                $this->setFlashMessage('Failed to restore course', 'error');
                $this->redirect('/lecturer/dashboard');
            }
        }
    }
